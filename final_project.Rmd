...
To start the evaluation I create three models based on algorithms best suited to a non-linear multi-variable evaluation.  The inital three models will be created utilizing methods `gbm` (Gradient Boosted Machine), `rpart` (Decision Tree), and `rf` (Random Forest) with cross validation.
...
out_sample <- 1 - (sum(fit_RF == testing$classe)/length(fit_RF))
paste0("The out of sample error rate for the final rf based model is: ", round(out_sample, digits = 2), "%")
```
...
fit_final <- predict(mod_RF, newdata = test_full[,1:45])

FinalPredictionResults <- data.frame(
  problem_id=test_full$problem_id,
  predicted=fit_final
)
